/* 生成配图核对页：每个配图模板出 2 个随机变式，
   ...
   用法：
     node tools/_preview_figs.js                         默认 2022 批配图题
...
/* 批次前缀：默认 2022 批，--set=icas21y2m 可核对 2021 批 */
...
h += '<h1>' + SETNAME + ' · 简化 SVG 配图核对</h1>';
h += '<div class="tip">左边是原题截图，右边是学生实际看到的题干 + SVG 配图。' +
     '重点看：图对不对、和原题是不是同一件事、选项图能不能分辨。</div>';

/* 有主图的 + 只有选项图（noFigure + optionsSvg）的都要看：后者最容易"图即答案" */
let figs = list.filter(t => new RegExp('^' + SET).test(t.id) &&
  (t.diagram || (t.answer && t.answer.optionsSvg)));
if (ONLY) figs = figs.filter(t => ONLY.indexOf(t.id) >= 0);
figs.forEach(t => {
  h += '<h3>' + t.id + ' · ' + (t.title || '') + ' · <code>' +
...
const noFig = list.filter(t => t.noFigure && new RegExp('^' + SET).test(t.id));
if (noFig.length) {
